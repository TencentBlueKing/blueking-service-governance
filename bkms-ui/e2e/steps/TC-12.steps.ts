 ...
 */
/**
 * TC-12 Helm 构建配置侧边栏：业务语义步骤。
 */
import { Given, Then, When } from '../fixtures/fixtures';

Given('我在 Helm 应用的基本信息页', async ({ pages }) => {
  await pages.appDetailPage.buildConfig.gotoBaseInfo();
});

When('我保存有效 Helm 构建配置', async ({ pages }) => {
  await pages.appDetailPage.buildConfig.saveValidBuilderConfig();
});

When('我打开 Helm 构建配置编辑侧栏', async ({ pages }) => {
  await pages.appDetailPage.buildConfig.openBuilderConfigSideslider();
});

When('我取消 Helm 构建配置编辑', async ({ pages }) => {
  await pages.appDetailPage.buildConfig.cancelBuilderConfigEdit();
});

When('我提交无效 Helm 构建配置', async ({ pages, userData }) => {
  userData.helmBuilderConfigInvalidSubmitted = await pages.appDetailPage.buildConfig.submitInvalidBuilderConfig();
});

When('我在 Helm 构建配置侧栏切换来源', async ({ pages }) => {
  await pages.appDetailPage.buildConfig.switchBuilderConfigSource();
});

Then('Helm 构建配置保存成功并关闭侧栏', async ({ pages }) => {
  await pages.appDetailPage.buildConfig.expectBuilderConfigSaveCompleted();
});

Then('Helm 构建配置侧栏应展示必填校验提示', async ({ pages, userData }) => {
  if (userData.helmBuilderConfigInvalidSubmitted === false) {
