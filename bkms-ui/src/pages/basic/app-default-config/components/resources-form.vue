    ...
    <div class="bg-[#FFF]">
      ...
      >
        <TableColumn
          ...
          min-width="180"
        >
          <template #default="{ row }: { row: ResourcesRuleOutputObj }">
            <Popover
              v-if="getResourceText(row.spec)"
              placement="top"
            >
              <span class="cursor-default border-b border-dashed border-[#979BA5]">{{
                getResourceText(row.spec)
              }}</span>
              <template #content>
                <div
                  v-for="line in getResourceTips(row.spec)"
                  :key="line"
        ...
        >
          <template #default="{ row }: { row: ResourcesRuleOutputObj }">
            <span class="text-[12px]">{{ row.spec?.replicas ?? '--' }}</span>
          </template>
        </TableColumn>
        <TableColumn
          field="envTypes"
...
<script lang="ts" setup>
  ...
  import { Button, Message, PopConfirm, Popover, Tag } from 'bkui-vue';
  ...
  import ResourcesRuleSideslider from './resources-rule-sideslider.vue';

  import type { AppSpecSliderScene } from './app-spec-scope';
  import type { ResourcesRuleOutputObj } from '~/@types/v1/app-spec';
  ...
  const ruleList = ref<ResourcesRuleOutputObj[]>([]);
  const sliderVisible = ref(false);
  const sliderScene = ref<AppSpecSliderScene>('addConfig');
  const editingRule = ref<null | ResourcesRuleOutputObj>(null);
  ...
  /** 四种环境类型都已配置时，不允许再新增（以列表全部规则为准，不受编辑态影响） */
  const isAddDisabled = computed(() => {
    const selectedEnvTypes = new Set(ruleList.value.flatMap(item => item.envTypes || []));
    return ALL_ENV_TYPES.every(envType => selectedEnvTypes.has(envType));
  });

  /**
   * 查询工作空间资源规格默认规则列表
   * GET /workspaces/{workspaceID}/app-spec/resources
   */
  async function fetchRuleList() {
    ...
    loading.value = true;
    try {
      const result = await AppSpecService.listWorkspaceAppSpecResourcesRules({
        workspaceID: spaceStore.currentSpace,
      });
      // 接口 data 可能为空数组；v1Fetch 已解包 data 字段
  ...
  /** 确认删除后调用接口，成功则刷新列表 */
  async function handleDelete(row: ResourcesRuleOutputObj) {
    if (!spaceStore.currentSpace || !row.id) return;
    await AppSpecService.deleteWorkspaceAppSpecResourcesRule({
      workspaceID: spaceStore.currentSpace,
      ruleID: row.id,
    });
  ...
  function handleEdit(row: ResourcesRuleOutputObj) {
    openSlider('editRule', row);
  }

  /** 打开资源规格侧滑，三种入口共用同一组件 */
  function openSlider(scene: AppSpecSliderScene, rule: null | ResourcesRuleOutputObj = null) {
  ...
  // 空间 ID 就绪后再拉列表，避免 onMounted 时 currentSpace 仍为空
  watch(
...
<style lang="postcss" scoped>
  ...
  /* 表头：背景白色 + 默认文字颜色 */
  :deep(.resources-table .vxe-header--column) {
    background-color: #fff;
    color: #4d4f56;
  }

  /* 表格正文：默认文字颜色 */
  :deep(.resources-table .vxe-body--column .vxe-cell) {
    color: #4d4f56;
  }
</style>
