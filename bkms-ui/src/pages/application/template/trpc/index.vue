...
<template>
  <ScrollLayout content-class="w-full relative">
    ...
    <AppConfig
      v-else-if="step === 3"
      ref="appRef"
      :editor-title="appConfig.appModelSpec?.trpcSpec?.fileName || ''"
      :value="appConfig"
    />
    <!-- 创建结果 -->
    <Result
...
<script lang="ts" setup>
  ...
  import type { AppModelSpecInput, CreateAppRequest, TrpcSpecInput, VariableInput } from '~/@types/v1/app';
  ...
  const space = computed<string>(() => {
    const s = route.params?.space;
    if (Array.isArray(s)) return s[0] || '';
    return (s as string) || '';
  });

  // 表单数据
  type TrpcCreateAppRequest = Omit<CreateAppRequest, 'appModelSpec'> & {
  ...
  } as TrpcCreateAppRequest);

  watch(
    () => appConfig.value.appModelSpec?.trpcSpec?.language,
    newLanguage => {
      if (newLanguage) {
        const fileName = `trpc_${newLanguage}.yaml`;
        appConfig.value.appModelSpec.trpcSpec.fileName = fileName;
      }
    },
    { immediate: true },
  );
  ...
  const nextStep = async () => {
    ...
    if (formRef.value) {
      ...
      Object.assign(appConfig.value, value);
    }
    emits('next');
  };
  // 上一步
  ...
  async function prepareAppConfigData() {
    ...
    appConfig.value.appModelSpec.trpcSpec.fileContent = data.content.trim();
    if (data.command.length) {
      appConfig.value.appModelSpec.command = data.command;
    }
    if (data.args.length) {
      appConfig.value.appModelSpec.args = data.args;
    }
    if (data.env.length) {
      appConfig.value.appModelSpec.envVars = data.env as unknown as VariableInput[];
    }

    if (appConfig.value.buildConfig.sourceType === 'pipeline') {
      // 流水线构建 -> 清除源码仓库配置
      Object.assign(appConfig.value.buildConfig, { repoBuildConfig: null });
    } else if (appConfig.value.buildConfig.sourceType === 'codeRepository') {
      // 代码仓库 -> 清除流水线配置
      Object.assign(appConfig.value.buildConfig, { pipelineBuildConfig: null });
    }
  }
