...
var _ = Describe("PatchGameDeployment TAF", func() {
	...
	Describe("Extra Objects", func() {
		...
		It("should return ConfigMap as extra object", func() {
			...
			Expect(configMapObj.GetName()).To(Equal(devmode.ConfigMapResourceName("test-taf-app")))
		})
		...
		It("should include taf-start.sh in ConfigMap data", func() {
			_, extraObjs, err := devmode.PatchGameDeployment(baseGD, validConfig)
			Expect(err).NotTo(HaveOccurred())
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(found).To(BeTrue())
			Expect(data).To(HaveKey(devmode.KeyTafStartScript))
		})
	})
	...
	Describe("Full Integration", func() {
		It("should successfully patch GameDeployment with all dev mode components", func() {
			config := &devmode.Config{
				...
				EnvType:        devmode.EnvTypeDevelopment,
				AppType:        bkmsapp.AppTypeTAF,
				AppName:        "my-taf-service",
				WorkPath:       "/data/bkms/dev-mode/taf",
				StartupCommand: "./my-taf-service --config=/etc/config.yaml",
			}
			...
			// 验证 ConfigMap 已添加
			Expect(extraObjs).To(HaveLen(1))
			Expect(extraObjs[0].GetName()).To(Equal(devmode.ConfigMapResourceName("my-taf-service")))

			// 验证 Volume 已添加
			volumeFound := false
			for _, v := range result.Spec.Template.Spec.Volumes {
				if v.Name == devmode.ConfigMapResourceName("my-taf-service") {
					volumeFound = true
					break
				}
			}
			...
			for _, m := range mainContainer.VolumeMounts {
				if m.Name == devmode.ConfigMapResourceName("my-taf-service") {
					mountFound = true
					Expect(m.MountPath).To(Equal(devmode.TafMountPath))
					break
				}
			}
			...
			// 验证启动命令
			Expect(mainContainer.Command).To(Equal([]string{devmode.TafInitScriptPath}))
		...
		It("should use taf WorkPath for mount path calculation", func() {
			config := &devmode.Config{
				Enabled:        true,
				EnvType:        devmode.EnvTypeDevelopment,
				AppType:        bkmsapp.AppTypeTAF,
				AppName:        "my-taf-service",
				StartupCommand: "./my-taf-service",
			}
			...
			Expect(mountPath).To(Equal("/data/bkms/dev-mode/taf/configmap-scripts"))

			// 验证启动命令使用正确的路径
			Expect(mainContainer.Command).To(Equal([]string{"/data/bkms/dev-mode/taf/configmap-scripts/init.sh"}))
		})
	})
})
