...
// Config is the typed TAF frameworkConfig.
type Config struct {
	FileName string `json:"fileName" validate:"required"`
	FilePath string `json:"filePath" validate:"required"`
}

// DecodeConfig strictly decodes a frameworkConfig map into a TAF config.
func DecodeConfig(cfg map[string]any, version int) (Config, error) {
	...
	if err != nil {
		return Config{}, errors.Wrap(err, "TAF")
	}
	if err = configValidate.Struct(dest); err != nil {
		return Config{}, errors.Wrap(err, "validate TAF frameworkConfig")
	}
	return dest, nil
}

// ConfigMapFromLegacy copies file metadata from the compatibility DTO.
// FileContent stays outside frameworkConfig.
func ConfigMapFromLegacy(cfg appmodel.TafConfig) map[string]any {
