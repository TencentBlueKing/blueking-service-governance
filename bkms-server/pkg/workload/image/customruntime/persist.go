...
// PersistManager 在构建配置保存成功后 get_or_create 自定义镜像记录
...
// PersistAfterSave 对判定为自定义的 builder / runner 做 get_or_create，仅新建时触发快照
//
...
func (m *PersistManager) PersistAfterSave(ctx context.Context, workspaceID string, cfg *imagebuild.Config) error {
	...
	builderErr := m.persistOne(ctx, workspaceID, ImageTypeBuilder, platformCfg.BuilderImage)
	runnerErr := m.persistOne(ctx, workspaceID, ImageTypeRunner, platformCfg.RunnerImage)
	return stderrors.Join(builderErr, runnerErr)
}

// persistOne 对单条 builder / runner 做路径判定后 get_or_create，仅新建记录时触发快照
func (m *PersistManager) persistOne(ctx context.Context, workspaceID string, imageType ImageType, image string) error {
	...
	if err != nil {
		log.Errorf(ctx, "match custom runtime image %s in workspace %s failed: %v", ref.Name, workspaceID, err)
		return err
	}
	if !matches {
		return nil
	...
	isNew, err := m.upsertRecord(ctx, workspaceID, imageType, ref.Name)
	if err != nil {
		return err
	}
	return m.refreshIfNew(ctx, workspaceID, ref.Name, isNew)
}

// upsertRecord 按 workspace + type + name get_or_create，返回本次是否新建
func (m *PersistManager) upsertRecord(
	ctx context.Context, workspaceID string, imageType ImageType, imageName string,
) (bool, error) {
	_, err := m.store.GetByWorkspaceTypeAndName(ctx, workspaceID, imageType, imageName)
	if err != nil && !errors.Is(err, ErrCustomRuntimeImageNotFound) {
		log.Errorf(ctx, "get custom runtime image %s/%s/%s failed: %v", workspaceID, imageType, imageName, err)
		return false, err
	}
	isNew := err != nil
	...
	if err = m.store.Upsert(ctx, &Image{
		...
		log.Errorf(ctx, "upsert custom runtime image %s/%s/%s failed: %v", workspaceID, imageType, imageName, err)
		return false, err
	}
	return isNew, nil
}

// refreshIfNew 仅为新建记录初始化 TAG 快照；已存在或未装配 snapshotSvc 时跳过
func (m *PersistManager) refreshIfNew(ctx context.Context, workspaceID, imageName string, isNew bool) error {
	if !isNew || m.snapshotSvc == nil {
		return nil
	}
	if _, err := m.snapshotSvc.RefreshWorkspaceSnapshots(ctx, workspaceID, imageName); err != nil {
		log.Errorf(ctx, "refresh workspace snapshots for image %s failed: %v", imageName, err)
		return err
	}
	return nil
}
