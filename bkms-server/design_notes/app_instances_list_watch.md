...
**本阶段状态**：N2 已落地全量拉取、互斥校验、全量投影失败跳过与北极星合并。北极星拉取失败时分页与全量均整次 List 失败。
...
```json5
{
  ...
  "object": {} // AppInstanceOutputObj；DELETED 可仅含 id 等定位字段
}
```
...
- List handler：`pkg/workload/instance/handler/instance.go`（`ListAppInstances` 编排）、`list.go`（绑定 / 部署记录 / 投影 / 北极星）
