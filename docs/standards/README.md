...
|------|------|------|--------|-------------------|
| 前端 | Vue 3 + TypeScript + Vite | [frontend-vue3.md](frontend-vue3.md) | vue, typescript, vite, pinia | `bkms-ui` |
| 接口 | Gin + Swagger/OpenAPI | [api-swagger.md](api-swagger.md) | gin, swagger, openapi, rest | `bkms-server` |
| 后端 | Go + Gin REST | [backend-gin.md](backend-gin.md) | go, gin, rest | `bkms-server` |
...
| 改接口契约/鉴权/联调（`bkms-server`） | 安全相关红线节 + 接口规范相关节 | 前端全文、质量全文 |
| 改后端业务逻辑（`bkms-server`） | 后端规范相关节 + 安全相关红线节 | 前端全文、质量评分量表 |
| 新建敏感接口/鉴权/加密逻辑 | 安全规范对应红线整节 + 检查清单 | 无关前端/质量长文 |
| Code Review / 质量评分 | 质量入口「本次必读决策表」+ 相关质量分册（检查维度等）；**以及**按变更路径（diff）加载前端 / 接口 / 后端规范相关节；涉及鉴权/输入/密钥时加载安全红线对应节 | 质量评分量表（非评分任务）、无关端全文 |

## Agent 加载策略

| 需求类型 | 应加载的规范 |
|---------|------------|
| 任何涉及业务代码的需求 | 按「加载预算」读安全规范的**相关节**；非每次全文 |
| 涉及 `bkms-ui` 页面/组件 | 按节加载 `frontend-vue3.md` 对应章节 |
| 涉及 `bkms-server` API 定义/联调 | 按节加载 `api-swagger.md` 对应章节 |
| 涉及 `bkms-server` 业务逻辑 | 按节加载 `backend-gin.md` 对应章节 |
| 触发 code-review / 质量评分 | 质量决策表 + 相关分册，并按 diff 路径加载前端/接口/后端相关节与安全红线相关节 |
...
| 六、两层代码架构 | frontend-vue3.md#六两层代码架构 |
| 七、Vue 3 最佳实践 | frontend-vue3.md#七vue-3-最佳实践 |
| 八、UI 组件使用规范 | frontend-vue3.md#八ui-组件使用规范 |
...
| 三、注解与 Handler 规范 | api-swagger.md#三注解与-handler-规范 |
| 四、数据类型与 JSON 契约 | api-swagger.md#四数据类型与-json-契约 |
...
无——前端（Vue 3）、接口（Swagger）、后端（Gin）三端均已匹配预设。
