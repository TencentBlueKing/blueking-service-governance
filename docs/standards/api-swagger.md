...
> 安全红线横切适用，见 `docs/standards/README.md` 选用表中的安全规范。  
> 后端分层与 Handler 写法见本仓已选用的后端规范。
...
| 前端消费 | 以项目 HTTP 封装为准（手写类型 / openapi-codegen 等） | **禁止**本文件强制 vitest 或某一 codegen |
...
| 路径参数 | 与本服务既有 JSON 风格一致，同服务不混用。绿场推荐 camelCase | `/users/{userId}` |
