...
| 类型 | 决策（摘要） | 详见 |
|------|-------------|------|
| 禁止 | 写死业务目录；在页面/组件里再 new 一套 HTTP 客户端；手改 Generated SDK | 二、六 |
...
| 禁止另起客户端 | 所有请求走仓库已有 HTTP 封装（前端根下 `src/api/*` 或等价）；禁止在页面/组件里再 `create` 一套客户端（axios / fetch / ky 皆然） |
...
│  - 组织 UI 和交互逻辑                            │
│  - 调用 Services 层获取数据                       │
│  - 不在页面里直接 new HTTP 客户端                  │
├────────────────────────────────────────────────┤
│  Services 层（桥接层）                            │
│  - services/{module}.ts                         │
│  - 封装 HTTP / 契约调用                          │
│  - 按后端契约做字段映射（若 JSON 与前端命名不同）  │
│  - 错误处理、业务工具函数                         │
└────────────────────────────────────────────────┘
```
...
### 附录：Generated 层（仅 codegen 仓）

仅当仓库存在 OpenAPI/Proto codegen 产物时适用：在 Services 之下增加 Generated SDK 层；有则禁止手改生成物，只重新生成。无 codegen 时不要把本附录当成现网默认分层。
