...
> 待修正：`bkms-cli` 的 `pnpm lint`（lint）在单元边界内找不到对应入口，未纳入验证表，详见 [`AGENTS.md`](../../AGENTS.md)。
> 待修正：`bkms-dockerfile-generator` 的 `make build-all compress`（build）在单元边界内找不到对应入口，未纳入验证表，详见 [`bkms-cli.yml`](../../.github/workflows/bkms-cli.yml)。
> 待修正：`bkms-dockerfile-generator` 的 `pnpm lint`（lint）在单元边界内找不到对应入口，未纳入验证表，详见 [`AGENTS.md`](../../AGENTS.md)。
> 待修正：`bkms-server` 的 `make build-all compress`（build）在单元边界内找不到对应入口，未纳入验证表，详见 [`bkms-cli.yml`](../../.github/workflows/bkms-cli.yml)。
> 待修正：`bkms-server` 的 `pnpm lint`（lint）在单元边界内找不到对应入口，未纳入验证表，详见 [`AGENTS.md`](../../AGENTS.md)。
> 待修正：`bkms-ui` 的 `make build-all compress`（build）在单元边界内找不到对应入口，未纳入验证表，详见 [`bkms-cli.yml`](../../.github/workflows/bkms-cli.yml)。
> 待修正：`bkms-ui` 的 `make test`（test）在单元边界内找不到对应入口，未纳入验证表，详见 [`bkms-cli.yml`](../../.github/workflows/bkms-cli.yml)。
> 待修正：`libs/bkms-adapter` 的 `make build-all compress`（build）在单元边界内找不到对应入口，未纳入验证表，详见 [`bkms-cli.yml`](../../.github/workflows/bkms-cli.yml)。
> 待修正：`libs/bkms-adapter` 的 `pnpm lint`（lint）在单元边界内找不到对应入口，未纳入验证表，详见 [`AGENTS.md`](../../AGENTS.md)。
> 待修正：`libs/bkms-adapter` 的 `make test`（test）在单元边界内找不到对应入口，未纳入验证表，详见 [`bkms-cli.yml`](../../.github/workflows/bkms-cli.yml)。
