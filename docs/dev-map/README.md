...
> **项目事实**：本仓体量较大（Go + Vue3 monorepo，2000+ 源文件），首版按 AST-only 模式生成
> （`graphify update . --no-cluster`），未运行全量语义抽取（docs/papers/images 子图谱与
> community 聚类）。如需更丰富的语义关联，可运行 `/graphify . --update` 补充语义抽取。
