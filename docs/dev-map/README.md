...
```bash
_ws="${CURSOR_PROJECT_DIR:-${CLAUDE_PROJECT_DIR:-}}"
export GRAPHIFY_NO_BACKUP=1
unset GRAPHIFY_OUT _graph_unit
if [ -n "$_ws" ]; then
  _git="$(git -C "$_ws" rev-parse --show-toplevel 2>/dev/null || true)"
  if [ -d "$_ws/docs/dev-map" ]; then
    _graph_unit="$_ws"
  elif [ -n "$_git" ] && [ "$_ws" != "$_git" ]; then
    _d="$(dirname "$_ws")"
    _graph_unit="$_ws"
    while :; do
      if [ -d "$_d/docs/dev-map" ]; then
        _graph_unit="$_d"
        break
      fi
      [ "$_d" = "$_git" ] && break
  ...
  export GRAPHIFY_OUT="$_graph_unit/docs/dev-map"
  graphify update "$_graph_unit" --no-cluster   # AST-only，无 LLM；或按 skill 跑全量 /graphify $_graph_unit
fi
```
