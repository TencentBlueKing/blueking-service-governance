...
// 对应 cmd/app/instance/
var _ = Describe("App Instance", Ordered, Label("readonly"), func() {
	BeforeAll(func() {
	...
	Context("List", func() {
		...
		It("app instance list without --app exits with non-zero code and output contains required", func() {
			cli.Run("app", "instance", "list", "--env", "test").
		...
		It("app instance list without --env exits with non-zero code and output contains required", func() {
			cli.Run("app", "instance", "list", "--app", envCfg.AppID).
				ExpectFailure().ExpectOutputContains("required")
		})
	})
})
