...
var _ = Describe("App Create Validation", Ordered, Label("readonly"), func() {
	BeforeAll(func() {
	...
	DescribeTable("input validation",
		func(file, expectedErr string) {
			cli.Run("app", "create", "-f", framework.TestdataPath("app", file)).
				ExpectFailure().ExpectOutputContains(expectedErr)
		},
		// required 校验
